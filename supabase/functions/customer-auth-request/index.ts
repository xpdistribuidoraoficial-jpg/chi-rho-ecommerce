...
import {
  ...
  consumeLimit,
  jsonResponse,
  metaConfigurationAvailable,
  normalizeBrazilianWhatsapp,
  secureFingerprint
} from "../_shared/customer-auth.ts";

const genericSuccess = {
  message: "Se os dados estiverem aptos para acesso, enviaremos as instruções pelo WhatsApp."
};
...
Deno.serve(async (request: Request) => {
  ...
  if (!metaConfigurationAvailable()) {
    return jsonResponse({
      error: "O acesso por WhatsApp ainda não está disponível. Tente novamente mais tarde.",
  ...
  try {
    ...
    if (!canonical) return jsonResponse({ error: "Informe um WhatsApp válido com DDD." }, 400, origin);
    ...
    const [phoneLimit, ipLimit] = await Promise.all([
      consumeLimit("request_phone", phoneHash, 3, 900, 60, 900),
      consumeLimit("request_ip", ipHash, 12, 900, 0, 900)
    ]);
    if (!phoneLimit.allowed || !ipLimit.allowed) {
      const retryAfter = Math.max(phoneLimit.retryAfter, ipLimit.retryAfter, 60);
    ...
    const authResponse = await fetch(`${url}/auth/v1/otp`, {
      ...
      body: JSON.stringify({ phone: `+${canonical}`, create_user: true }),
      signal: AbortSignal.timeout(15000)
    });
    if (!authResponse.ok) {
      console.error("Customer OTP delivery failed", authResponse.status);
    ...
    console.error("Customer OTP request failed", code);
    return jsonResponse({ error: "Não foi possível enviar o código agora. Tente novamente em alguns instantes." }, 503, origin);
  }
});
